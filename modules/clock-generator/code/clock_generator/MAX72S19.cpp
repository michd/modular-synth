...
MAX72S19::MAX72S19(uint8_t pinChipSelect, uint8_t pinDataOut, uint8_t pinClock) {
...
void MAX72S19::begin() {
	...
	::pinMode(_pinDataOut, OUTPUT);
	::pinMode(_pinClock, OUTPUT);
	::digitalWrite(_pinDataOut, 0);
...
uint8_t MAX72S19::_mapChar(char inputChar) {
	switch (inputChar) {
		...
		/* Commented out 'cause not needed in this project
		case 'a':
...
void MAX72S19::_setRegister(uint8_t reg, uint8_t data) {
	_beginTransmission();
	shiftOut(_pinDataOut, _pinClock, MSBFIRST, reg);
	shiftOut(_pinDataOut, _pinClock, MSBFIRST, data);
	_endTransmission();
}
